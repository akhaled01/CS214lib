public class Book {
    ...
    public Book (String t, String a1, String a2, String p, int yP, String ISBN, long aN){
        ...
        publisher = p;
        yearPublication = yP;
        //checking if the ISBN passed is 13-digits, otherwise setting it to default value
        if (ISBN.length() == 13) {
            isbn = ISBN;
        }
        else {
        ...
        //checking if the aN passed is more than 1000, otherwise setting it to default value
        if (aN > 1000) {
            accessionNum = aN;
        }
        else {
    ...
    public void setIsbn (String ISBN){
        //set method for isbn, checks if ISBN is 13-digits first, otherwise sets it to default value
        if (ISBN.length() == 13){
            isbn = ISBN;
        }
        else {
    ...
    public void setAccessionNum (long aN){
        //set method for accessionNum, checks if aN is more than 1000, otherwise sets it to default value
        if (aN > 1000){
            accessionNum = aN;
        }
        else {
